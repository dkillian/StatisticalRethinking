...
Null hypothesis: p = 50%. The event is equally likely to have occurred in the treatment or control group, signifying no treatment effect
...
```{r}
# out <- data.frame(rbind(prior, likelihood, numerator, posterior)) %>%
#   set_names(nm=c("10%", "20%", "30%", "40%", "50%", "60%", "70%", "80%", "90%")) %>%
#   round(3) %>%
#   mutate(metric=c("prior","likelihood","likelihood*prior", "posterior")) %>%
...
ot <- data.frame(p, likelihood, prior, numerator, posterior) %>%
  round(3)
